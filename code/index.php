...
const CONFIG_STORED = 'AutoIndex.conf.php';
const CONFIG_GENERATOR = 'config.php';

//paths for files that will be included
const PATH_TO_CLASSES = './classes/';
const PATH_TO_LANGUAGES = './languages/';
const LANGUAGE_FILE_EXT = '.txt';

//filenames of template files
const GLOBAL_HEADER = 'global_header.tpl';
const GLOBAL_FOOTER = 'global_footer.tpl';
const TABLE_HEADER = 'table_header.tpl';
const TABLE_FOOTER = 'table_footer.tpl';
const EACH_FILE = 'each_file.tpl';
 ...
 */
const IN_AUTOINDEX = true;

$_GET = array_change_key_case($_GET);
$_POST = array_change_key_case($_POST);

if (ini_get('zlib.output_compression') == '1') //compensate for compressed output set in php.ini
{
 ...
 * @return string
 */
function simple_display(string $text, string $title = 'Error on Page'): string
{
...
try {
    ...
    foreach ($config as $key => $item) /* Go through each config setting, and set a constant with each setting's
    ...
    if (BANNED_LIST && @is_file($config->__get('banned_list'))) //make sure the user is not banned
    {
        ...
        if (count($only_these_ips) > 0) {
            ...
            if (DirectoryList::match_in_array($ip, $banned_ips) ||
