...
if (count($_POST) >= count($strings) + count($numbers)) {
    ...
    foreach ($strings as $setting) {
        ...
        if (in_array($setting, $directories) && !preg_match('#/$#',
                $_POST[$setting])) //make sure there is a slash at the end of directories
        {
            $_POST[$setting] .= '/';
        }
...
<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?action=config">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                Base Directory: <input type="text" name="base_dir" value="<?php if ($settings['base_dir'] != 'false') {
                    echo $settings['base_dir'];
                } ?>"/>
                <p class="small">
                    This is the folder that will be the root of the directory listing.
                    <br/>This will be the starting point for the script. Nothing above this directory can be viewed, but
                    its subfolders can.
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                Icon Path: <input type="text" name="icon_path" value="<?php if ($settings['icon_path'] != 'false') {
                    echo $settings['icon_path'];
                } ?>"/>
                <p class="small">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <input type="checkbox" name="show_dir_size"
                       value="true"<?php if ($settings['show_dir_size'] != 'false') {
                    echo ' checked="checked"';
                } ?> /> Show Directory Size
                <p class="small">
                    If this box is checked, the total size of directories will be shown under size (all the folder's
                    contents will be added up).
                    <br/>Otherwise, it will display "[dir]" under size.
                    <br/>NOTE: If you are trying to index many files (meaning a few thousand), you will notice a speed
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <input type="checkbox" name="search_enabled"
                       value="true"<?php if ($settings['search_enabled'] != 'false') {
                    echo ' checked="checked"';
                } ?> /> Enable Searching
                <p class="small">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                Template Directory: <input type="text" name="template"
                                           value="<?php if ($settings['template'] != 'false') {
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <input type="checkbox" name="use_login_system"
                       value="true"<?php if ($settings['use_login_system'] != 'false') {
                    echo ' checked="checked"';
                } ?> /> Enable Login System
                <br/><input type="checkbox" name="must_login_to_download"
                            value="true"<?php if ($settings['must_login_to_download'] != 'false') {
                    echo ' checked="checked"';
                } ?> /> Users must login to view/download
                <br/>User List: <input type="text" name="user_list"
                                       value="<?php if ($settings['user_list'] != 'false') {
                                           echo $settings['user_list'];
                                       } ?>"/>
                <p class="small">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                Age for "New" Icon: <input type="text" name="days_new" size="3"
                                           value="<?php if ($settings['days_new'] != 'false') {
                                               echo $settings['days_new'];
                                           } ?>"/> days
                <p class="small">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                Number of file entires per page: <input type="text" name="entries_per_page" size="3"
                                                        value="<?php if ($settings['entries_per_page'] != 'false') {
                                                            echo $settings['entries_per_page'];
                                                        } ?>"/>
                <p class="small">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                Image Thumbnail Height: <input type="text" name="thumbnail_height" size="3"
                                               value="<?php if ($settings['thumbnail_height'] != 'false') {
                                                   echo $settings['thumbnail_height'];
                                               } ?>"/> pixels
                <p class="small">
                    This is a feature that will show thumbnails next to images. (NOTE: GDlib 2.0.1 or higher is
                    required)
                    <br/>Setting it to 0 will disable this feature, and setting it to any other number will set the size
                    of the thumbnail.
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <input type="checkbox" name="force_download"
                       value="true"<?php if ($settings['force_download'] != 'false') {
                    echo ' checked="checked"';
                } ?> /> Pipe downloaded files though the PHP script
                <p>Bandwidth Limit: <input type="text" name="bandwidth_limit" size="3"
                                           value="<?php if ($settings['bandwidth_limit'] != 'false') {
                                               echo $settings['bandwidth_limit'];
                                           } ?>"/> KB/s</p>
                <p class="small">
                    This contains the max download speed for files. The above checkbox needs to be checked for this to
                    work.
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <p><input type="checkbox" name="anti_leech" value="true"<?php if ($settings['anti_leech'] != 'false') {
                        echo ' checked="checked"';
                    } ?> /> Anti-Leech</p>
                <p class="small">
                    When downloading a file, this will check to make sure the referrer the browser sends matches the
                    website's URL.
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                ...
                <p>
                    The following items contain the path and filename to the file where the data for that feature will
                    be stored.
                    <br/>Leave it blank to turn off that feature.
                </p>

                <p>Hidden Files List: <input type="text" name="hidden_files"
                                             value="<?php if ($settings['hidden_files'] != 'false') {
                                                 echo $settings['hidden_files'];
                                             } ?>"/>
                    <br/><span class="small">
	...
	<br/>The contents of the list are editable when you login as an admin.
                ...
                <p>Access Log File: <input type="text" name="log_file"
                                           value="<?php if ($settings['log_file'] != 'false') {
	...
	<br/>and generate statistics when you login as an admin.
                ...
                <p>File/Folder Description File: <input type="text" name="description_file"
                                                        value="<?php if ($settings['description_file'] != 'false') {
                                                            echo $settings['description_file'];
                                                        } ?>"/>
                    <br/><span class="small">
	...
	<br/>File/Folder descriptions are editable when you login as an admin.
                ...
                <p>Download Count File: <input type="text" name="download_count"
                                               value="<?php if ($settings['download_count'] != 'false') {
                                                   echo $settings['download_count'];
                                               } ?>"/>
                    <br/><span class="small">
	...
	<br/>The count is automatically increased when a file is downloaded.
                ...
                <p>Banned User List: <input type="text" name="banned_list"
                                            value="<?php if ($settings['banned_list'] != 'false') {
                                                echo $settings['banned_list'];
                                            } ?>"/>
                    <br/><span class="small">
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <input type="checkbox" name="archive" value="true"<?php if ($settings['archive'] != 'false') {
                    echo ' checked="checked"';
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <input type="checkbox" name="parse_htaccess"
                       value="true"<?php if ($settings['parse_htaccess'] != 'false') {
                    echo ' checked="checked"';
    ...
    <table width="650" cellpadding="8">
        <tr>
            <td>
                <p>MD5 calculation max size: <input type="text" name="md5_show" size="3"
                                                    value="<?php if ($settings['md5_show'] != 'false') {
                                                        echo $settings['md5_show'];
                                                    } ?>"/> MB</p>
                <p class="small">
                    Setting this to 0 will disable this feature, and setting it to any other number will set the maximum
                    size of a file to allow users to find the md5sum of (in megabytes).
    ...
    <p>
        ...
        <br/>If it cannot (for example if it does not have write permission in the directory) the config file will be
        downloaded, and you will have to upload it to your server.
